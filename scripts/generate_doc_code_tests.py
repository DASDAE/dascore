"""
Generate pytest modules from executable Python fences in dascore/docs/*.qmd.

Exclude generated API pages and honor document- and chunk-level evaluation switches.
Each page becomes one test, preserving block order, hoisting future imports, and adding
source-line comments. A shared conftest runs tests from each page's directory and
manages non-interactive matplotlib figures.

Regeneration replaces tests/test_autogenerated_doccode to remove stale artifacts;
...
# Source qmd files live under dascore/docs/.
DOCS_PATH = REPO_ROOT / "dascore" / "docs"
...
def no_printed_panels(source_qmd: str):
    ...
    # either: dascore/docs/tutorial/patch.qmd imports dascore's, which is rich's,
    ...
    def guard(wrapped):
        ...
        def guarded(*args, **kwargs):
            # Somewhere other than the cell's own output, so there is no
            # panel it could be hiding -- a file, or a buffer an example
            # goes on to show the contents of. Naming a standard stream
            # is not that: the text lands in the cell either way.
            stream = kwargs.get("file")
            if stream is None or stream in (sys.stdout, sys.stderr):
                for arg in args:
...
def iter_source_qmd_files(base_path: Path = DOCS_PATH) -> list[Path]:
    ...
    # Mirror every qmd under dascore/docs/, except generated API docs.
    return [
        path
...
def get_output_path(
    ...
    # Keep the generated tree shaped like dascore/docs/, but swap the filename to
