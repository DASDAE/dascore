...
def distributions(tmp_path_factory):
    ...
    for name in (
        "dascore/proc/__pycache__/compiled.nbc",
        "dascore/proc/__pycache__/compiled.nbi",
        "tests/test_autogenerated_doccode/test_generated.py",
    ):
        path = source / name
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("Generated test or compiler cache must not ship.")
...
class TestDocumentationDistribution:
    ...
    def test_contents(self, distributions):
        ...
        for names in (source_names, wheel_names):
            ...
            assert not any(
                "generated." in name
                or "page_files" in name
                or name.endswith(".ipynb")
                or "__pycache__" in name
                or "test_autogenerated_doccode" in name
                for name in names
            )
