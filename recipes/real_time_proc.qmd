...
In this section, we define the window size and step size required for [rolling](`dascore.proc.rolling.rolling`) mean processing. Additionally, we establish the desired wait time after each run by using the `sleep_time_mult` parameter, which acts as a multiplier coefficient for the number of seconds in each patch.
...
while True:
    ...
    for j, patch in enumerate (sp[len_last_sp:]): 
        ...
        # Save results 
        # Just the name: source_path is absolute, and os.path.join would
        # discard output_data_dir and overwrite the input file.
        file_name = os.path.basename(sp.get_contents()["source_path"][patch_num])
