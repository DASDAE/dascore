...
:::{.callout-note}
CI doesn't download registry files one by one; it checks out the whole test_data repo once and builds a single test-data cache shared by all operating systems (see `.github/actions/prime-test-data-cache`). The cache invalidates automatically whenever data_registry.txt changes. If the cache ever needs a manual reset (e.g. if `DATA_VERSION` changes without a registry edit), increment the `cache-number` passed to that action.
:::
