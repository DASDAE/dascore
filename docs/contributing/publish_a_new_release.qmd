...
Each PR carries its own entries under the required `## Changelog` heading of the [pull request template](https://github.com/DASDAE/dascore/blob/dev/.github/pull_request_template.md), already written and categorized by the author and checked by CI, so drafting the notes mostly means collecting and grouping them. Each bullet names one of six categories, and carries `**breaking**` when it can break code written against the last released version:
...
Group the collected bullets under these sections, omitting any that end up empty:

- Breaking Changes (every `**breaking**` entry, repeated from its own category so upgraders see them first)
- Added
- Changed
- Deprecated
- Removed
...
One extra step applies to the first release drafted after this change. Work merged before the pull request template gained its release-note sections was instead described in `docs/changelog.qmd`, and that curated text is richer than those PR bodies alone. It remains in git history: find the commit that reduced the page to a stub, then read the revision before it.
