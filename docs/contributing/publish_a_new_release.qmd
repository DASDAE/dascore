...
For the first release drafted after the changelog was retired, one extra step applies. `docs/changelog.qmd` used to accumulate curated entries for unreleased work, and for the PRs merged before the template gained its release-note sections that text is richer than the PR bodies alone. Read it before drafting:
