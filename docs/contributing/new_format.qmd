...
up filed under a name that means something else — ProdML's `AcquisitionId`
spent a release as `acquisition_id` meaning the DASDAE identity, and
OptoDAS's `experiment` spent one as `experiment_id`.
...
3. Return one payload with those values plus `dims`, `shape`, and `dtype`.
