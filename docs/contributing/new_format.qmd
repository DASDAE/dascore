...
Formats that read stored per-sample coordinate arrays must not use bare `get_coord(array)` for the exact path because its tolerant uniformity inference can snap small jitter. For strictly monotonic arrays, use `CoordSegmented.from_array(values, tolerance=0, units=...)`; this preserves every value, returns a `CoordRange` for a truly uniform array, and exposes internal sampling changes through `CoordSegmented` when present. Non-monotonic stored arrays may fall back to an array coordinate because they cannot be represented as segmented monotonic runs.
