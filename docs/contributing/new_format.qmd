...
First, we create a new io module called 'jingle' in DASCore's io module (dascore/io/jingle). Make sure there is a \__init__.py file in this module whose docstring describes the basics of the format and lists any non-obvious implementation details that might help debug/improve the parser.
...
class JingleV1(FiberIO):
    ...
    def get_format(self, path, **kwargs):
        ...
        Returns a tuple of (format_name, version) if the file is a
    ...
    def scan(self, path, **kwargs):
        ...
        [`PatchSummary`](`dascore.PatchSummary`) objects
        from the [dascore.core.attrs](`dascore.core.attrs`) module.
        """
...
All 4 methods are optional; some formats will only support reading, others only writing. If `get_format` is not implemented the format will not be auto-detectable, meaning you will have to manually pass the format to [`dascore.spool`](`dascore.spool`).


:::{.callout-note}
Note that each of these methods should have `**kwargs`. This allows certain formats to have special keywords.
:::
...
Rather than using paths for the IO methods as shown above, it is better practice to write a `FiberIO` which supports the [python stream interface](https://docs.python.org/3/library/io.html#io.BufferedIOBase) or an opened HDF5 file in the form of a `h5py.File` object. There are a few reasons for this:
...
* It is usually more efficient since open-file handles can be reused.
...
:::{.callout-warning}
The `PytableReader` and similar formats which were based on pytables have been dropped so that PyTables is no longer a dependency of DASCore. See [#482](https://github.com/DASDAE/dascore/discussions/482) and [#310.](https://github.com/DASDAE/dascore/pull/310)
:::
