...
Most single-patch `read()` implementations share a tail as well: apply the caller's dimension selections, drop the patch when the selection empties it, then attach attrs. [`build_patches`](`dascore.io.build_patches`) does exactly that, so a reader usually ends with:

```python
return dc.spool(build_patches(coords, data_node, attrs, attr_cls=JinglePatchAttrs, time=time, distance=distance))
```
