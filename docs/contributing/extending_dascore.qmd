...
Some operations are [`PatchProcessor`](`dascore.core.processor.PatchProcessor`) classes, which plan metadata apart from their array kernel. A backend package can register its own kernel without changing the operation. The kernel takes the processor, the data, and whatever the class's `plan` returns (`abs` plans nothing; `normalize` plans `axis`):
