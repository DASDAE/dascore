...
A [`PatchProcessor`](`dascore.PatchProcessor`) subclass is a whole operation. Its fields are the parameters; subclassing registers it and generates its patch function, so the class is written once:
...
patch = dc.get_example_patch()
out = Scale(3)(patch)  # or Scale.patch_function(patch, factor=3)
assert out.equals(Scale.patch_function(patch, 3))
```

Calling a processor checks the patch, runs it, and records history and lineage ids, exactly as a decorated patch function does. Its fingerprint is its validated fields, so `Scale(3)` and `Scale(3.0)` are one operation.
...
- `derive(patch)` returns the result's metadata from a patch without data, through the coord manager and `patch.new`.
- `plan(patch, out)` returns what the kernel needs, as ints, floats, bools, tuples of those, or numeric arrays.
...
A backend package can give a processor its own kernel without changing the operation. The kernel takes the processor, the data, and whatever the class's `plan` returns (`abs` plans nothing; `normalize` plans `axis`):
