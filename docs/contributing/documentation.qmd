...
This step also writes the parts of the site which are generated from the code rather than the API docstrings — the quarto config itself, and the [inventory](../tutorial/inventory.qmd) data-model diagram — so quarto cannot render the site until it has run.
