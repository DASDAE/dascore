...
Not every function can tell cheaply, and those which cannot still return a new patch: the test has to cost less than the copy it saves, so it is made from metadata (units, dimension names, a mask already computed) and never from comparing two data arrays. Treat the identity as an optimization to benefit from, not a check to write logic against.
