...
# Operations which have nothing to do

Patches are immutable, so a processing function normally returns a new patch. When the function is asked for something the patch already is, though, it hands back the patch it was given rather than building a copy:

```{python}
import dascore as dc

patch = dc.get_example_patch()  # its distance coordinate is already in metres

# Converting to the units already set is not work, so nothing is done.
assert patch.convert_units(distance="m") is patch

# Neither is transposing to the dimension order already in place.
assert patch.transpose(*patch.dims) is patch
```

Nothing was done, so nothing is written to the patch's history either. One call mapped over a spool whose patches are not all alike therefore leaves some of them with a history entry for it and some without, and `Spool.chunk` compares histories when it merges — so such a spool refuses to merge until it is chunked with `conflict="drop"`. This is not new: `transpose`, `select` and `sort_coords` have always worked this way.

The check has to cost less than the copy it saves, so it is made from metadata — units, dimension names, a mask already computed — and never from comparing two data arrays; a function which cannot tell that cheaply still returns a new patch. Treat the identity as an optimization, not a guarantee to write logic against.
