...
This changes STFT and stacked-tile metadata: `{dim}_start` / `{dim}_stop` previously held unitless sample indices; they now hold physical cell edges in the dimension's units. Code that reads these coordinates must account for the new values and units. Reconstruction retains its original indices privately in `_tile_index_{dim}`; inverse STFT and reassembly continue to use those indices.

Selection and raw decimation retain the selected bounds. Operations that build a new grid, including interpolation, resampling, filtered decimation, padding, and rolling windows, drop them. Translation and unit conversion transform them with the dimension. Waterfall image extents use the outer cell edges: half a step beyond the first and last labels, or the explicit pair when attached.
