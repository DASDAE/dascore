...
## Directory Indexer
The `DBDirectoryIndexer` is used to track the contents of a directory which
contains fiber data. It creates a small, hidden database index (sqlite by
default; duckdb and parquet backends are also available) at the top of the
directory which can be efficiently queried for directory contents
