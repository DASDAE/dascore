...
# concatenate
Similar to `chunk`, [`Spool.concatenate`](`dascore.Spool.concatenate`) is used to combine patches together. However, `concatenate` ignores gaps and overlaps along the concatenation axis — it joins each group's patches in the order of that dimension (spool order when the dimension has no step to tell its orientation) — and can even be used to create new patch dimensions. It partitions patches as `chunk` does (by kind, dimensions, and the other dimensions' identities), so patches which cannot be concatenated together simply land in separate outputs, and it polices the remaining attributes with the same `conflict` argument. Like `chunk`, it produces a lazy, plan-backed result.
