...
- Only the directories *inside* the spool are parsed. The spool's own root contributes nothing, so `dc.spool(".../experiment=demo")` does not pick up `experiment`; point the spool at a parent to keep that partition. File names are not parsed either — an extension cannot be told apart from a value that ends in one, so a name like `acquisition_key=XX.R2D1..RAW` would be ambiguous.
