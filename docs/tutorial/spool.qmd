...
`processing_id` is also indexed. Attribute queries match the stored source ID, including on a selected view; a trim clears the ID in presented contents because the loaded patch records the selection and carries a different value. Built or chunked spools may have no `patch_id`, and moving a source invalidates its derived ID until the directory is rescanned. See [Patch identity](patch.qmd#patch-identity) for the identity rules.
