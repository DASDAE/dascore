...
[to_datetime64](`dc.utils.time.to_datetime64`) which tries to convert most types of inputs expressing date times
to the proper numpy type. [to_timedelta64](`dc.utils.time.to_datetime64`) performs a similar function for
...
# filtered_distance = patch.pass_filter(distance=(1, 2))
```

 However, its not clear what the units of the provided range are!
...
The `Spool` manages a group of `Patch` instances. These can be in memory, on
disk, or a remote resource.

![DASCore Patch and Spool](../_static/patch_n_spool.png){#fig-patch_n_spool}
