...
DASCore provides first class support for units through the `units` module. Units (or rather quantities) can be created with  [get_quantity](`dascore.units.get_quantity`).
...
Many of DASCore's processing functions support units/quantities as shown above. See the [units section](patch.qmd#units) in the Patch tutorial for examples.
