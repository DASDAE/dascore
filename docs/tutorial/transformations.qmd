...
In DASCore, transformations are operations which change the units of a patch. Transforms can be found in the [transform module](`dascore.transform`) or access via the `Patch.tran` attribute.
...
transformed = patch.tran.dft(dim="time")
...
:::{.callout-note}
The transformed dimension names change; "time" becomes "ft_time" indicating the domain of the dimension has changed. The units are also updated. See the
:::
...
real_transform = patch.tran.dft(dim='distance', real=True)
print(real_transform.get_coord("ft_distance"))
```

Of course, this operation can be undone using the [idft](`dascore.tran.idft`)

```{patch}
# transform back to time domain, take only real component.
patch_2 = transformed.tran.idft().real()
...
fk_patch = patch.tran.dft(patch.dims)
