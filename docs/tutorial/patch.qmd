...
If you want to suppress new history entries for a block of operations, use `set_config(patch_history="disabled")`. Existing history is preserved, but DASCore will stop appending new entries until that config context exits.
