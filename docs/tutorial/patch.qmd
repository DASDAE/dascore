...
A [`Patch`](`dascore.core.patch.Patch`) manages data and its associated coordinates and metadata. Most often, these are 2D arrays with `time` and `distance` coordinates.
...
This example patch contains some artificially generated data. They are mostly used for simple demonstrations and testing.
...
We first download a small example fiber file from a URL in the DASCore library (you need an internet connection). Next, we read it into a [spool](spool.qmd) object then get the first (and only) patch. We will learn more about spools in the next section.
...
print(f"Data contents are {patch.data}")
```

:::{.callout-note}
The data arrays are generally read-only. This means you can't modify them, but should first make a copy.
