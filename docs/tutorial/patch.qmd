...
`Patch.attrs` only stores non-coordinate metadata. This includes things like the data source id, data units, and observing-system fields such as `gauge_length` or `pulse_width`.
...
# Update existing attribute 'data_source_id' and create new attr 'new_attr'
pa1 = pa.update_attrs(data_source_id='EXP1.R2D1..RAW', new_attr=42)
```

```{python}
#| echo: false
assert pa1.attrs.data_source_id == "EXP1.R2D1..RAW"
