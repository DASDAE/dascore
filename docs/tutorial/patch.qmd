...
DASCore Patches offer a hew shortcuts for quickly accessing commonly used information:
...
dist_order = np.array([20., 10., 15.])
patch_dist = patch.order(distance=dist_order)

assert np.all(dist_order == patch_dist.get_array("distance"))
```
