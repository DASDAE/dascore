...
There are two types of entities included in coordinates:
 ...
 - Dimensions (dims): the name and labels associated with a patch dimension, which is an axis of the data array (e.g. time, distance)

 - Coordinates (coords): the name and labels for non-dimension data which can be associated with a dimension (e.g. coordinates "latitude" and "longitude" are often associated with dimension "distance")

All dimensions are coordinates but not all coordinates are dimensions.
:::

The rest of the `CoordManager` features are mostly used internally by DASCore, but you can read more about them in the [Coordinate Tutorial](coords.qmd).
