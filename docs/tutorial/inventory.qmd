...
A group, a geometry column, and a track each become a coordinate under their own name, so one name is one coordinate: a group may not take the name of a structural coordinate (`time`, `distance`, `channel`, `instrument_distance`, `acquisition_key`), of a track (`optical_components`, `geometry`, `coupling`, `labels`), or of any recognised coordinate label, whether or not this inventory's coordinate reference system declares it; the full set is [`RESERVED_GROUP_NAMES`](`dascore.core.inventory.RESERVED_GROUP_NAMES`). A group and a geometry column cannot share a name either.
