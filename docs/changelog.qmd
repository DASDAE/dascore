...
- Removed coordinate-management helpers and fields from `PatchAttrs`, including `coords`, `dim_tuple`, `rename_dimension()`, `coords_from_dims()`, `__setitem__`, and dynamic coordinate attributes such as `attrs.time_min`. Use `Patch.coords` and `Patch.update_coords(...)` for coordinate data and coordinate changes.
- `PatchAttrs.flat_dump()` is now attrs-only and aliases `PatchAttrs.model_dump(...)`. Use `Patch.summary.flat_dump(dim_tuple=...)` when flattened coordinate summary fields such as `time_min` or `distance_step` are needed.
