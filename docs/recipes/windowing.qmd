...
With `mode="stack"`, tiles remain separate. Windowed dimensions become tile-center axes, `{dim}_start` and `{dim}_stop` record physical cell edges in the dimension's units, and tile samples use `{dim}_offset`. Process the stack as a patch, then [`reassemble`](`dascore.Patch.reassemble`) it.
