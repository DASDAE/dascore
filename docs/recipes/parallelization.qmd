...
The simplest way to process spools in a patch in parallel is using the [dascore.Spool.map](`dascore.BaseSpool.map`) method in conjunction with the Python standard library module [concurrent.futures](https://docs.python.org/3/library/concurrent.futures.html).
...
The `ThreadPoolExecutor` from the same module will also work, but due to python's GIL may not provide too much of a speed-up.
