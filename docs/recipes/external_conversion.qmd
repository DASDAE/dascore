...
A source patch larger than `block_size` is read in several windows along the merged dimension rather than whole, so one block never has to hold a whole large file. Unset, it takes the configured `xarray_block_size` (256 MiB by default); pass `0` to read every source patch as one block, which makes any selection touching a patch read all of it. A format which cannot hand back a window reads whole whatever the setting says, since splitting it would read the file once per block.
