...
`coord_variants` and `meta_data.patch_count` are summaries rather than records. Triggers update them in the transaction that changes the rows they count, so every connection keeps them exact, including one opened before an upgrade, and a spool's coordinate names, kinds and units, and a root spool's length, cost the same on a large archive as on a small one. A coordinate stated under several value kinds sorts by the kind most patches state.
...
The schema version is validated before any mutation. An unrelated or incomplete database raises an error with instructions to delete and rebuild it; DASCore does not silently repair it. An index of the previous schema version is upgraded in place from its own rows, in one transaction, without reading any source file. An older index is rebuilt automatically by the directory indexer — the index is a disposable cache whose truth is the files. An index written by a newer DASCore is left untouched and raises an error: upgrade DASCore, or give the older version its own `index_path`.
