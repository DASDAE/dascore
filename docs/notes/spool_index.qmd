...
`_dtype` and `_data_size` (the samples in the data array, the product of its shape) are private for the same reason: chunk's merge-compatibility grouping and conflict policing compare every public column, and patches of different element types — or, far more often, different lengths — must still merge. A row states no size when it does not know one. A patch assembled from several, a piece cut from one, and a row a selection trims — including a residual selection, which is applied at load rather than folded into the envelopes — all describe a patch which does not exist yet, and how many samples it comes out with is known only once it is loaded: those rows leave `data_size` null rather than repeating a member's count. An output which is one whole, untrimmed member *is* that patch, so it keeps that patch's size. The chunk planner and selection both operate on the full relation (see the [Spool Chunking](spool_chunking.qmd) and [Spool Selection](spool_selection.qmd) notes).
