...
`coord_variants` and `meta_data.patch_count` are summaries rather than records. Every write updates them in the transaction that changes the rows they count, so a spool's coordinate names, kinds and units, and a root spool's length, cost the same on a large archive as on a small one.
