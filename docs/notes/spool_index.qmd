...
(root / "data_source_id=XX.R2D1..RAW" / "cable=A").mkdir(parents=True)
sub = root / "data_source_id=XX.R2D1..RAW" / "cable=A"
dc.get_example_patch().io.write(sub / "tag=raw.h5", "DASDAE")

spool = dc.spool(root).update()
row = spool.get_contents().iloc[0]
assert (row["data_source_id"], row["cable"], row["tag"]) == ("XX.R2D1..RAW", "A", "raw")
