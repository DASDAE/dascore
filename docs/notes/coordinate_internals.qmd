...
[`CoordSummary`](`dascore.core.coords.CoordSummary`) contains `min`, `max`, `step`, `dtype`, `units`, associated `dims`, length, and an exact coordinate fingerprint. The canonical construction paths are `coord.to_summary()` and `coord_summary_from_data(...)`, both of which start from a live coordinate or full value array.
