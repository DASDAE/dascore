...
`select` and `order` retain dimensions; scalar `sel`/`isel` indexers reduce them. Small label queries evaluate only the needed samples of compact ranges. Bulk queries use vectorized pandas lookup when that avoids repeated Python searches. Grids whose floating-point resolution can introduce duplicate labels also use the full index to enforce pandas' uniqueness rules. Integer and temporal slices remain compact. `select` and `order` also preserve compact floating ranges. To match xarray exactly, `sel`/`isel` floating slices evaluate the selected labels and become a range only when canonicalization preserves every value; this can require memory proportional to the number of selected samples, including temporary position and value arrays. String label slices and exact matching in `sel` follow pandas' rules, while coordinate-level `select` retains DASCore's wildcard and range behavior.
