...
- **`concatenate` and `stack`**: the output carries the first patch's attributes, with kind values it lacks filled from the others, and its history with the operation's own entry appended.
