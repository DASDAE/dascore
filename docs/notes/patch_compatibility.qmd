...
Because a missing value matches anything, "same kind" is not transitive — `"a"` matches `""` matches `"b"`, yet `"a"` conflicts with `"b"` — and operations that combine more than two patches resolve this in two ways. `concatenate` and `stack` run through their patches in order with an *accumulated* kind: a value any kept patch knows binds the patches after it, so a run never ends up holding two values for one attribute. `chunk` must partition a whole spool at once, so a patch missing a value joins the fully specified kind it is consistent with only when there is exactly one such kind; with none, or with several conflicting candidates, it stays in a partition of its own (shared with patches missing the same values and agreeing on the rest). That keeps plans deterministic and order-independent, and never merges an unlabelled file between two acquisitions.
