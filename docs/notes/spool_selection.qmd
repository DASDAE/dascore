...
A selection pushed into a reader records the trim in the loaded patch's history and `processing_id`. Native-unit, sample-index, and relative bounds retain the same call representation as direct `Patch.select`. Quantity bounds retain the spool's existing canonical SI representation, so their history and processing IDs can differ from a direct call with differently spelled units even when the selected data are equal. Attribute predicates, including `processing_id`, always query indexed source metadata; they do not query provenance generated when a selected patch loads.
