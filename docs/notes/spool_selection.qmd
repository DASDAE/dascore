...
A selection pushed into a reader records the trim in the loaded patch's history and `processing_id`. History records normalized selectors: open `...` bounds become `None`, two-element lists become tuples, and quantity bounds use canonical SI units. These representations can differ from the syntax recorded by a direct `Patch.select` call even when the selected data are equal. Attribute predicates, including `processing_id`, always query indexed source metadata; they do not query provenance generated when a selected patch loads.
