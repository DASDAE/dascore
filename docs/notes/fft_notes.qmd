...
These notes provide the reasoning for DASCore's [fft](`dacore.tran.fft`) module implementation.

## Summary {#sec-summary}

- DACore multiplies numpy's fft output by the coordinate spacing in order to:
...
- If the scaled fft is divided by the signal duration, the amplitude spectrum magnitudes correspond to the magnitude of the composite harmonics.
...
A related property of the @eq-3 is that the zero frequency represents the DC offset, or integral, of the original function. If set $\xi=0$ we see:
...
Right away you might notice some reasons @eq-4 can't meet our expectations. First, nowhere are the units of $a$'s axis multiplied by the summation. Since the exponential term must be unit-less, $a$ must have the same units as $A$. Next, the energy can't be preserved since the number of elements in the summation increases as $n$ increases. Lastly, $A(0) = \sum_{m=0}^{n-1} a_m$ which is not the integral, but the sum, of the $a$ (it doesn't consider sample spacing).
...
As expected, both have spikes at $\pm f_0$ (5Hz), but the amplitude of the 1000 sample function 2X larger than the 500 sample function. This isn't entirely surprising; from @eq-6 we expect the amplitude to tend toward infinity as the signal moves from discrete to continuous ($n \rightarrow \infty$). However, this scaling is typically not useful. Certainly, it won't help us meet our expectations.

## Scaling

One approach to fix the differences in scaling is to divide each fft by the number of samples. After which, they have nearly identical spectral amplitudes.
...
What if, drawing inspiration from @eq-1, we simply scale the output units by the function's sample spacing? After all, the spacing and the number of samples is directly related by
...
The transforms with different numbers of points still have the same magnitude at $\pm f_0$. Checking their energy conservation:
...
The energy is conserved, and the expected units are produced. However, we would need to divide by the total signal duration $T$ in order for the spectrum magnitudes to correspond to the time-domain amplitude of harmonics in the new basis (@eq-7).


## Zero Frequency

What about the zero frequency representing the integral over the signal domain? It is hard to say with a zero-mean signal like the sine wave used in the previous section. We could use a ramp function, such that $f(x) = x$. Using the 5 second signal from before, which forms a triangle with area of 12.5, we get the following transforms:
...
All our requirements/expectations of the dft if we simply multiply numpy's fft output by the sample spacing ($dx$). If we want the amplitude spectrum magnitudes to correspond to the amplitude of the composite harmonics, we just need to divide by the signal duration.

:::{.callout-note}
Although not mentioned in the text, we also need to be careful to undo the frequency shift and divide by the sample spacing before performing the inverse transform.
:::
Patch methods [dft](`dascore.tran.fft.dft`) and [idft](`dascore.tran.fft.idft`) handle this for you.
