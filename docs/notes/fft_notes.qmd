...
## Summary {#sec-summary}
 ...
 - DACore multiplies the fft output by the coordinate spacing in order to:
 ...
 - DASCore also performs a frequency shift so the output axis labels are sorted.
...
@eq-1 has many other properties covered by many texts. One property that is relavent to DASCore's implementation is [Parseval's identity](https://en.wikipedia.org/wiki/Parseval%27s_identity) which, in 1D, can be stated as:
...
#| fig-cap: "n Scalled Amplitude Plot"
...
It doesn't. And what about the units? As we saw above, $A$ in @eq-4 would have the same units as $a$. So what if, drawing inspiration from @eq-1, we simply scale the output units by the function sample spacing? After all, the spacing and the number of samples is directly related by
...
#| fig-cap: "dt Scalled Amplitude Plot"
...
They still have the same amplitude, and checking their energy conservation:
...
The energy is conserved, and the expected units are produced.


## Zero Frequency

What about the zero frequency representing the integral over the signal domain? It is hard to say with a zero-mean signal like a sine wave. We could use a triangle though, such that $f(x) = x$. Using the 5 second signal from before, which forms a triangle with area of 12.5, we get the following transforms:

```{python}
#| code-fold: true
#| label: triangle_fft_plot
#| fig-cap: "Triangle scaled fft"
...
print(np.abs(tri_fft1_scaled).max())
plt.show()
```

Both outputs have a zero frequency very near the expected value of 12.5.

## Conclusions

All our requirements/expectations of the dft if we simply multiply numpy's fft output by the sample spacing. We would also need to be careful to undo the frequency shift and divide by the sample spacing before performing the inverse transform.
