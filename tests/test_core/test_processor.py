...
class TestNumpyKernel:
    ...
    def test_numpy_kernel_requires_data(self, patch):
        """A numpy-only computation is still an operation on data."""
        with pytest.raises(PatchDataError, match="holds no data"):
            self.NumpyOnly()(patch.drop_data())

    @pytest.mark.parametrize("noop", [False, True])
    def test_bypass_keeps_data_type(self, patch, noop):
        """Bypassing provenance retains the operation's result metadata."""

        class Tagged(PatchProcessor):
            """A NumPy operation with a declared result type."""

            name = None
            data_type = "tagged"

            def numpy_kernel(self, data):
                """Either return the array or double it."""
                return data if noop else data * 2

        out = Tagged()._run(patch, record=False)
...
class TestReconcileWithData:
    ...
    class DropFirstNegativeRows(PatchProcessor):
        ...
        def reconcile(self, result, out):
            ...
            return out.new(coords=coords).to_patch(data)
