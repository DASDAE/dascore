...
class TestOneRowArithmetic:
    ...
    def test_a_derived_table_is_already_canonical(self):
        ...
        for out in derived:
            again, _ = _canonical(out.runs, out.labels, out.dtype)
...
class TestFromExisting:
    ...
    def test_from_rows_round_trip(self):
        ...
        rebuilt = NumericND.from_rows(
            coord.runs, coord.labels, dtype=coord.dtype, units=coord.units
        )
    ...
    def test_get_coord_rebuilds_from_runs(self, mixed):
        ...
        assert out == mixed
        assert out.fingerprint() == mixed.fingerprint()
    ...
    def test_summary_dump_round_trip(self, mixed, holed):
        """A dumped summary keeps its run table, hashes and all."""
...
class TestHashing:
    ...
    def test_both_folds_agree(self, monkeypatch, kind, mixed):
        ...
        coords = {
            ...
            "int": concat_tables(
                NumericND.from_run(start=0, step=3, shape=(5,)),
                NumericND.from_run(start=100, step=3, shape=(5,)),
            ),
            "timedelta": NumericND.from_run(
                start=np.timedelta64(0, "s"), step=np.timedelta64(2, "s"), shape=(9,)
            ),
        }
        coord = coords[kind]
    ...
    def test_equal_after_split_and_concat(self, mixed):
        ...
        assert concat_tables(*parts).fingerprint() == mixed.fingerprint()
...
class TestUpdates:
    ...
    def test_legacy_grid_fields_need_their_bounds(self):
        ...
        summary = dict(dtype="int64", min=0, max=None, step=1, step_numerator=1)
        with pytest.raises(ValidationError, match="both of its bounds"):
            dc.core.coords.CoordSummary(**summary)
        bad = dict(dtype="int64", min=0, max=10, step=1, step_denominator=0)
        with pytest.raises(ValidationError, match="positive denominator"):
            dc.core.coords.CoordSummary(**bad)
    ...
    def test_summary_states_the_grid(self):
        ...
        (run,) = summary.runs
        assert (run["num"], run["den"]) == (1953125, 2)
    ...
    def test_legacy_grid_fields_still_state_a_run(self):
        ...
        assert summary.runs is not None and len(summary.runs) == 1
...
class TestIndependentContracts:
    ...
    def test_summary_states_the_exact_grid(self):
        ...
        # 1/1024 s is 1953125/2 ns, which the run states as it is.
        assert summary.runs.tolist() == [(_to_tick(T0), 500, 1953125, 2, 0)]
