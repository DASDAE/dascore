...
class TestMergeAttrs:
    ...
    def test_drop_conflicts(self, random_patch):
        ...
        pa1 = attrs.update(tag="bill", data_source_id="UU.R2D1..RAW")
        pa2 = attrs.update(tag="bob", data_source_id="TA.R2D1..RAW")
        out = combine_patch_attrs([pa1, pa2], conflicts="drop")
        defaults = PatchAttrs()
        assert out.tag == defaults.tag
        assert out.data_source_id == defaults.data_source_id
