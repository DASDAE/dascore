...
class TestIntervalValueType:
    """One value type, refused in the subsystem's own vocabulary."""

    def test_the_refusal_reaches_the_caller(self):
        """The inventory refuses a boolean, and says why.

        The type it raises is not observable here: pydantic wraps whatever
        a validator raises, keeping the message. The message is therefore
        what the caller actually gets, so it is what is pinned.
        """
        adapter = TypeAdapter(inventory.LabelValue)
