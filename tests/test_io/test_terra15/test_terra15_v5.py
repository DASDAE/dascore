...
def terra15_v5_patch(terra15_v5_path):
    ...
    return dc.read(terra15_v5_path)[0]
...
class TestIsTerra15V5:
    ...
    def test_format_and_version(self, terra15_v5_path):
        """Ensure version"""
        name, version = Terra15FormatterV5().get_format(terra15_v5_path)
        assert (name, version) == (Terra15FormatterV5.name, Terra15FormatterV5.version)

    def test_not_terra15_not_hdf5(self, dummy_text_file):
        """Test for not even a hdf5 file."""
        parser = Terra15FormatterV5()
...
class TestScanTerra15V5:
    ...
    def test_basic_scan(self, terra15_v5_path):
        """Tests for getting summary info from terra15 data."""
        parser = Terra15FormatterV5()
        out = parser.scan(terra15_v5_path)
