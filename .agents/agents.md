...
Put the user-facing summary in the pull request's required `## Changelog` section instead, following [Changelog entries](../docs/contributing/general_guidelines.qmd) — that page is the one definition of the format. Write for the person reading the release notes: what changed for them and what to do about it, leaving reasoning and implementation detail to the diff. `.github/scripts/check_pr_changelog.py` is the parser CI runs.
