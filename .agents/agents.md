...
- `.qmd` under `dascore/docs/`; API docs come from docstrings. Update with any behavior or API change. Do not hard-wrap prose.
- Edit `scripts/_templates/_quarto.yml` for site structure; `dascore/docs/_quarto.yml` is generated.

```bash
python scripts/build_api_docs.py && quarto render dascore/docs
```

### Changelog

No changelog file, and do not add one — no `CHANGELOG.md`, `changelog.d/`, or "unreleased" sections. `dascore/docs/changelog.qmd` is a stub pinned by `tests/test_changelog.py`. Put the summary in the PR's required `## Changelog` section, formatted per "Changelog entries" in `dascore/docs/contributing/general_guidelines.qmd`; `.github/scripts/check_pr_changelog.py` is the parser CI runs.
