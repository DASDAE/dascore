...
3. Use that same executable with `-m dascore skill dascore` to load the current routing recipe, and with `-m dascore skills` to discover specialized workflows. Read relevant APIs with `-m dascore doc <name>` and find unfamiliar APIs or procedures with `-m dascore doc-search <query>`.
