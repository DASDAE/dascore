...
def _get_g1_h5_base_coords(resource, dims, extra_coords=None, snap=True):
    ...
    time = (
        get_coord(data=time_values)
        if should_snap(snap, "time")
        else NumericND.from_array(time_values)
    )
    ...
    sample_span = NumericND.from_array(np.atleast_1d(dc.to_timedelta64(ends - starts)))
    ...
    if should_snap(snap, "distance"):
        ...
        distance = NumericND.from_array(np.atleast_1d(distances), units="m")
    temperatures = np.atleast_1d(resource["temperatures"][...])
    temperature = (
        get_coord(data=temperatures, units="°C")
        if should_snap(snap, "temperature")
        else NumericND.from_array(temperatures, units="°C")
    )
    coords = {
        "time": time,
        "distance": distance,
