...
def _get_sampled_coord(h5file, coord_name: str, coord_len: int) -> BaseCoord | None:
    ...
    if values.dtype.kind in "iu" and (den != 1 or float(step).is_integer()):
        # Whole labels: an interval of whole units keeps them there, and so
        # does an exact ratio, which is DASCore's own spelling of a grid it
        # states in ticks. XDAS writes neither over an integer axis whose
        # interval is fractional -- it evaluates start + k * interval and
        # gets doubles -- so that axis is promoted below, as XDAS has it.
    ...
    rows = float_rows(dtype, values.astype(np.float64), lengths, float(step) / den)
    return NumericND.from_rows(rows, dtype=dtype)
