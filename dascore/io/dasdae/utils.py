...
def _coord_from_values(array, units, snap, name):
    """A coordinate from stored values; only a snapped dimension is fitted."""
    if should_snap(snap, name) or np.asarray(array).dtype.kind in "USO":
        return get_coord(data=array, units=units)
    return NumericND.from_array(array, units=units)
