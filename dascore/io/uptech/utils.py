...
def _get_time(resource):
    ...
    if len(values) > 1:
        ...
        # (ms/us/ns rather than s); the reader fits near-even labels only
        # when snap=True.
...
def _get_coords(resource, snap=True):
    ...
    time = (
        ...
        else NumericND.from_array(values)
    )
...
def _get_attrs_dict(resource) -> dict:
    ...
    return {
        ...
        "data_units": "1/s",
        # Uptech reports these in meters, which is what dascore expects.
