...
`new`, `update`, `update_attrs` and `set_dims` replace what a patch is made of. Inside an operation — the body of a patch function or a `PatchProcessor`, and the routes which combine patches — they add nothing, because the operation stamps its own result. Called outside one they say what happened: an array the patch was handed gets a random `data_id` (nothing names it) with `origin_id` kept, while replaced coordinates or attributes are a describable change and get a derived one. A call which changes nothing, and a call which states an ID itself, keep the IDs they were given. `PatchMeta`, which describes data it does not hold, is left to the routes which build it.
