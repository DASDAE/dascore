...
Fractional grids use their exact cadence and phase for gap decisions, including after a spool selection clips a run. Runs cut from one grid retain `step_exact`; `missing().positions()` reproduces the original grid's missing labels, and an absolute fill limit retains fractions of a nanosecond. Whole-tick grids keep their existing tolerance rounding.
