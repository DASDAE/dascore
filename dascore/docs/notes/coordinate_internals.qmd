...
Fractional grids use their exact cadence and phase for gap decisions, including after a spool selection clips a run. Runs cut from one grid retain `step_exact`; `missing().positions()` reproduces the original grid's missing labels, and an absolute fill limit supplied in seconds retains fractions of a nanosecond. Whole-tick grids keep their existing tolerance rounding, and filling with a timedelta limit keeps its half-nanosecond rounding allowance, so `fill_gaps(time=coord.step)` still fills a one-sample hole.
