...
- `get_coord(data=..., snap=False)` detects uniform runs in one stored monotonic array. A difference belongs to a run when it equals a neighboring difference; isolated differences become seams. With `step` declared (as `get_coord(data=..., step=...)` does at every length), every spacing must be a whole number of steps or construction raises, each run of consecutive positions is a grid of the step, and `missing()` counts the positions between runs. Past a thousand samples an array whose runs would outnumber a tenth of them keeps its labels as one stored run, with the declared step on the coordinate, so `missing()` answers the same on both sides of that guard.
