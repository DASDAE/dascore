...
|---|---|---|
| `meta_data` | index | Identifies the file and its schema version, and counts its patches |
...
| `patches` | patch within a source | Stores patch identity, the data array's type and sample count, and common time/distance envelopes |
...
| `patch_coords` | coordinate attached to a patch, and each run of a multi-run one | Links patches to coordinate definitions while retaining the coordinate name and dimensions; `run_index` 0 is the coordinate as a whole |
| `coord_variants` | distinct coordinate name, dtype, kind, units, and relativity | Counts the patches stating each variant, so coordinate discovery reads a few rows rather than every link |

`coord_defs` and `patch_coords` are deliberately separate. Many patches can share a distance coordinate, so `coord_defs` stores it once and `patch_coords` supplies the many-to-many attachment. Range coordinates receive an exact semantic fingerprint, reconstructed from the coordinate values when a scan did not supply one. Non-range coordinates only receive a merge-compatibility identity when the scan provides an exact fingerprint. This distinction prevents a matching envelope from being mistaken for matching coordinate values while retaining the deduplication needed by future merge planning. Each run's row states its grid in the run table's own terms (`num`, `den`, `offset`) beside its envelope, and an `origin` where a float run's grid is counted from somewhere other than its first label, as a slice's is, so a row rebuilds the coordinate the file holds rather than one a rounding away.
...
File-backed patches are identified by `(base_uri, source_path, source_patch_key)` instead; their rows resolve through readers rather than the registry.
...
The index retains source `processing_id` values, original numeric attribute and coordinate dtypes, and whether scalar attributes fully describe a source patch (history remains excluded). Temporal dtype names follow the existing summary normalization and temporal envelopes use nanoseconds; a temporal coordinate is counted in nanoseconds whatever unit it was read in, so there is one temporal dtype to retain rather than the file's own. Coordinate names are retained even when their values have no supported envelope representation; such names are not selectable. This metadata lets consumers distinguish rows they can reconstruct from those that require loading the source patch. Plan output rows require resolution through their members. Index version 15 added this reconstruction metadata; version 16 stores each range coordinate's exact grid (`step_numerator`, `step_denominator`, `origin_offset`, with `length` authoritative for the sample count), marks with `is_exact` the rows which rebuild every value of their coordinate, and names the envelope columns by storage type (`min_int`/`max_int`/`step_int` for nanosecond times and durations, `min_float`/`max_float`/`step_float` for numbers, `min_str`/`max_str` for text). Version 17 links a segmented coordinate to each of its runs as well as to itself: `patch_coords.run_index` is 0 for the coordinate as a whole, which every query about a patch's coordinate reads, and 1, 2, … for its runs. Version 18 adds `coord_variants` and `meta_data.patch_count`. Version 19 states each of those rows as the run it describes: `num`, `den` and `offset` replace `step_numerator`, `step_denominator` and `origin_offset` and are written per run rather than per range, `run_hash` holds that run's position-independent 64 bit hash (indexed, so runs shared between patches are found by a join), and the run's start and stop are its envelope columns, which the sign of `num` says which way round to read. A coordinate of one run states its grid on its own row and is linked to no runs, so an archive of contiguous patches still pays nothing; a coordinate of several states each run's grid on that run's row and is rebuilt from them exactly, holes and all, where before the row rebuilt the gapless range across them. There is no longer any cap on how many runs a coordinate may state. Gap reports and chunk planning read runs through a partial index holding just those links; chunk plans each run as a trimmed member, and plans and merge exports carry runs along. Because version 19 changes what a stored value means, an index written by an earlier DASCore cannot be read at all: a directory index deletes and rebuilds itself when opened, re-scanning every file behind it, and an index opened directly raises saying to delete it and rebuild.
