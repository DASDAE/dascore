...
`coord_variants` and `meta_data.patch_count` are summaries rather than records. Triggers update them in the transaction that changes the rows they count, so every connection keeps them exact, including one opened before an upgrade, and a spool's coordinate names, kinds and units, and a root spool's length, cost the same on a large archive as on a small one.
