...
An extra attr holds one value. A one-value array, list or tuple, which is how HDF5 and netCDF store a scalar, is read as that scalar; one holding several values is skipped with a warning, because an array belongs on the patch as a coordinate: `patch.update_coords(name=(dims, array))`, or `(None, array)` for one which rides no dimension.
