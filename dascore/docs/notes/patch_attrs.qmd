...
An attr is one value. Strings, bytes, booleans, numbers, `None`, numpy scalars (`datetime64` and `timedelta64` included), datetimes, timedeltas, enums and pint quantities and units with a scalar magnitude are attrs; a 0-d array is the scalar it wraps. Anything holding more than one value — an array with a shape, a list, a tuple, a set, a mapping, a dataframe or series, another model — is refused:

```python
patch.update_attrs(gauge_length=np.array([1.0, 2.0]))  # raises
```
...
A file written before this rule may store an array in its attr namespace. Reading one still works: the reader drops the values a patch attr cannot hold and warns once, naming them. The refusal is for a patch built in memory, where the caller can put the value where it belongs.
