---
keywords: [spool, chunking, overlapping chunks, gaps, archive]
title: Spool
execute:
  warning: false
---
...
`processing_id` is also indexed. Attribute queries match the stored source ID, including the empty string when no operations were recorded. A value trim clears the ID in presented contents because the loaded patch records the selection and carries a different value; sample selections defer it until loading. Built or chunked spools may have no `patch_id`, and moving a source invalidates its derived ID until the directory is rescanned. See [Patch identity](patch.qmd#patch-identity) for the identity rules.
...
Use [`Spool.iterate`](`dascore.core.spool.Spool.iterate`) to load upcoming patches in a background thread while processing the current patch:
