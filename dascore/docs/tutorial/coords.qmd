...
Float coordinates have no tick to be exact against, so a fraction step on one raises. Their spacing is still kept as a fraction -- `0.1` is `1/10` -- which `step_exact` reports and every label is computed from, so a float range no longer accumulates its step.
