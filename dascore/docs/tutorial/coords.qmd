...
[`get_coord`](`dascore.core.coords.get_coord`) builds a coordinate from a range or an array.
...
Every numeric coordinate is a [`NumericND`](`dascore.core.coords.NumericND`), holding grid runs or stored labels. Recognized uniform stretches become grid runs; other labels remain stored, even when their source was a uniform grid. Construct coordinates with `get_coord` and inspect `evenly_sampled`, `runs_count`, or `holes` for their structure.

`get_coord(data=...)` also reads a monotonic array whose spacings nearly agree as the grid they restate, which may move a label by a fraction of a step. [`NumericND.from_array`](`dascore.core.coords.NumericND.from_array`) is the factory which never does.
...
| `evenly_sampled` | Whether the coordinate holds one nonempty grid run |
| `dtype` | NumPy data type |
| `values` | Array of coordinate values |
| `units` | Physical units, if present |
| `has_values` | Whether the coordinate holds labels, rather than only a shape |
| `min()`, `max()` | Coordinate limits |
| `step` | The spacing every run shares (whole ticks for times and integers), or `None` when they disagree |
| `step_exact` | The spacing as an exact `Fraction` (seconds for time), or `None` for a float coordinate or when the runs do not share one |
| `runs_count` | Number of grid and stored runs |
| `holes` | Whether any run starts past where the run before it would have ended |

### Exact sampling rates

Time and integer ranges accept exact spacing as a `Fraction` or `(numerator, denominator)` tuple. For example, `(1, 1024)` seconds avoids the drift of a rounded nanosecond interval. Temporal labels floor their exact grid positions to nanoseconds; slicing, striding, reversal, and selection preserve the grid's phase. `step` reports the rounded whole-tick spacing.
...
Float coordinates reject fractional steps and return `None` for `step_exact`. Grid runs compute labels from an origin and integer indices, using multiplication or explicitly stored division terms. Slices preserve their parent's labels exactly.

A time coordinate is counted in nanoseconds whatever unit its labels were given in, so `coord.dtype` is always `datetime64[ns]` or `timedelta64[ns]`: a `datetime64[us]` array comes back as `[ns]`, holding the same instants. Two consequences are worth knowing. A date outside 1678-2262 has no nanosecond count and is refused with a `CoordError` naming it, where a coarser unit could hold it before. And a label finer than a nanosecond -- a `timedelta64[ps]` of 10 picoseconds, say -- is refused for the same reason rather than rounded onto the tick; picoseconds which are whole nanoseconds convert as they read.
...
`NumericND.from_array` preserves its labels exactly. Detection accepts a grid only when it reproduces every label, but does not recognize every possible grid formula. Declare known sampling with `start`, `step`, and `shape`. To intentionally move labels, use `snap()` to fit one grid or `fuse(tolerance=...)` to join nearby runs.

## Segmented coordinates

A coordinate of several runs preserves those blocks and the discontinuities between them. Use [`concat_coords(...)`](`dascore.core.coords.concat_coords`) when the blocks are known:
...
assert segmented.runs_count == 2 and segmented.holes
print(segmented.get_discontinuities("gaps"))
```

[`NumericND.from_array(...)`](`dascore.core.coords.NumericND.from_array`) preserves labels as recognized grid runs and stored stretches. It reports a `step` when the runs share a declared grid, and `None` otherwise. Use `patch.split_gaps()` before writing gapped dimensions to formats that cannot represent gaps; DASDAE version 2 stores them whole.

```{python}
from dascore.core.coords import NumericND

values = np.array([0.0, 1.0, 2.0, 7.0, 8.0, 9.0])
exact = NumericND.from_array(values, units="m")
assert np.array_equal(exact.values, values)
assert exact.runs_count == 2 and exact.holes
```

### Declaring a grid and finding missing samples

Labels alone do not identify missing positions: `[0, 2, 4]` fills a step-2 grid but misses two positions on a step-1 grid. Passing `step` declares that grid and rejects off-grid values. The declaration survives whether labels become grid runs or stay stored. [`missing()`](`dascore.core.coords.BaseCoord.missing`) reports unoccupied positions between the first and last sample, compactly grouping consecutive missing positions.

```{python}
coord = get_coord(data=[1, 3, 4, 10, 11, 12], step=1)
assert coord.runs_count == 3 and coord.step == 1
...
[`get_discontinuities`](`dascore.core.coords.BaseCoord.get_discontinuities`) reports the seams between runs, and inside a stored run every spacing that is not its declared step (or its median spacing when it declares none). Its `tolerance` is an excess over one step in coordinate units; pass [`GapTolerance.samples(k)`](`dascore.utils.gaps.GapTolerance`) to count steps instead, which is how `Spool.chunk` and `waterfall(gap_factor=)` judge the same spacing.
...
Updating `min`, `max`, or `step` retains the coordinate length and derives the remaining range values. Updating `data` instead rebuilds the run table from the new array.
...
`sort` permutes existing values and returns the matching permutation. `snap` changes values, while `fuse(tolerance=...)` chooses a simpler representation only within the requested displacement bound. The distinction is important whenever coordinate values are measurements rather than ideal labels.

`snap()` returns only a coordinate because it replaces values rather than permuting them. Use `CoordManager.snap` or `Patch.snap_coords` when data must be sorted with the coordinate before snapping. Snapping places no bound on how far an interior label moves; `fuse(tolerance=...)` re-fits one run at a time within the displacement bound it is given instead.
