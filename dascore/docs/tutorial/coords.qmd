...
| `step_exact` | The spacing as an exact `Fraction` (seconds for time), or `None` when the runs do not share one |
...
Use [`NumericND.from_array(...)`](`dascore.core.coords.NumericND.from_array`) to preserve an exact array. It reads the runs out of the spacings, so uniform input becomes one run and anything no grid describes is stored whole. Such a coordinate reports a `step` only when every run declares the same one, and `None` otherwise. `simplify(tolerance=...)` may fuse runs within an error bound, while `snap()` replaces values with one evenly sampled range. Use `patch.split_gaps()` before writing gapped dimensions to formats that cannot represent gaps; DASDAE version 2 stores them whole.
