...
Array inputs preserve their labels exactly. Detection accepts a grid only when it reproduces every label, but does not recognize every possible grid formula. Declare known sampling with `start`, `step`, and `shape`. To intentionally move labels, use `snap(tolerance=...)` to fit one grid or `fuse(tolerance=...)` to join nearby runs.
