...
# the rounded step would have drifted 1.8 ms by the end of the hour
assert time.max() - (t0 + time.step * (len(time) - 1)) == np.timedelta64(1843199, "ns")
```
...
## Coordinates with holes

A coordinate holds its labels as a tuple of *runs* — an evenly sampled [`Grid`](`dascore.core.coords.Grid`) described by numbers, or [`Labels`](`dascore.core.coords.Labels`) held as they are — so monotonic blocks and the discontinuities between them survive. Use [`concat_coords(...)`](`dascore.core.coords.concat_coords`) when the blocks are known:
...
Pass `snap=False` to preserve an exact monotonic array. It returns the simplest truthful coordinate, so uniform input becomes one evenly sampled run. A coordinate of several runs reports a `step` only when every run sits on the same one, and `None` otherwise. `fuse(tolerance=...)` may reduce the number of runs within an error bound, while `snap()` replaces values with one evenly sampled run. Use `patch.split_gaps()` before writing a dimension with holes to formats that cannot represent gaps; DASDAE version 2 stores them whole.

```{python}
values = np.array([0.0, 1.0, 2.0, 7.0, 8.0, 9.0])
exact = get_coord(data=values, units="m", snap=False)
assert np.array_equal(exact.values, values)
assert exact.runs_count == 2
```

### Declaring a grid and finding missing samples

Labels alone do not say which positions are missing: `[0, 2, 4]` fills a step-2 grid and misses two positions of a step-1 grid. Passing `step` with array data declares the grid. Every value must sit on it (an off-grid value raises rather than moves), each run of consecutive positions becomes a grid of that step, and [`missing()`](`dascore.core.coords.BaseCoord.missing`) reports the positions between the first and last sample that have no sample, one entry per hole rather than one per position.
...
A dense array whose runs would outnumber a tenth of its samples keeps its labels as one stored run with the declared step, and `missing()` answers the same. Without a declared step there is no grid to be missing from, so `missing()` reports nothing missing; evenly sampled coordinates are always complete. Filling the holes with samples is a patch operation, never a coordinate one. [`Patch.fill_gaps`](`dascore.Patch.fill_gaps`) places every run on one grid and writes a fill value (NaN by default) where no sample sits, so the coordinate becomes one evenly sampled run. Giving the dimension a limit as a keyword (`fill_gaps(time=10)` in coordinate units, or a count of missing samples with `samples=True`) fills only holes that narrow and leaves wider ones as seams. A hole's width is its missing samples times the step.
