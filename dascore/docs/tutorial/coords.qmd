...
Every numeric coordinate is a [`NumericND`](`dascore.core.coords.NumericND`): a table of evenly sampled *runs*. A plain range is one run, a coordinate with gaps in it is several, and values no grid describes -- jittered timestamps, unsorted or multidimensional arrays -- are held whole as a *stored* run. The four classes these used to be split across are removed in 1.0: build a coordinate with `get_coord`, and ask `evenly_sampled`, `runs_count`, or `holes` what it holds.
...
Float coordinates have no tick to be exact against, so a fraction step on one raises. A float step is the double it is, so `step_exact` is `None`; every label is `start + k * step` (or `k / rate` for an axis built by dividing), computed from the start rather than accumulated, and a slice holds exactly the doubles of its parent.
...
Array inputs preserve their labels exactly. Near-even values remain uneven unless an exact grid reproduces every value. If an instrument or calculation already supplies the start, sampling interval and count, construct the coordinate with those declarations. To intentionally fit measured labels, call the coordinate's `snap` method, with a `tolerance` to bound how far any label may move, or `fuse` to join runs within an error bound.
...
Use [`NumericND.from_array(...)`](`dascore.core.coords.NumericND.from_array`) to preserve an exact array. It reads the runs out of the spacings, so uniform input becomes one run and anything no grid describes is stored whole. Such a coordinate reports a `step` only when every run declares the same one, and `None` otherwise. `fuse(tolerance=...)` may fuse runs within an error bound, while `snap()` replaces values with one evenly sampled range. Use `patch.split_gaps()` before writing gapped dimensions to formats that cannot represent gaps; DASDAE version 2 stores them whole.

```{python}
from dascore.core.coords import NumericND

values = np.array([0.0, 1.0, 2.0, 7.0, 8.0, 9.0])
exact = NumericND.from_array(values, tolerance=0, units="m")
...
`snap()` returns only a coordinate because it replaces values rather than permuting them. Use `CoordManager.snap` or `Patch.snap_coords` when data must be sorted with the coordinate before snapping. Both take a `tolerance`, spelled as `Spool.chunk` spells its own: a number is a multiple of the step, a quantity or timedelta is a distance. With one, a coordinate is snapped only if no label moves further than that, and is otherwise left exactly as it is; this is what a reader's `snap=True` does, within the configured `snap_tolerance`.
