...
DASDAE files can be compressed and chunked through the `storage` option. A preset name is the shortest form; `"compressed"` applies gzip with HDF5's shuffle filter:

```{python}
patch.io.write(path, "DASDAE", storage="compressed")
```

A dict (or a [`DASDAEStorage`](`dascore.io.dasdae.DASDAEStorage`)) sets the options directly. `chunks` gives the samples per chunk for each dimension name; unnamed dimensions stay whole, and sizes past an array's length are clamped. Without `chunks`, h5py picks the chunk shape. Options apply to the data and to coordinates stored as arrays. The other fields pass straight to h5py, so `compression` accepts whatever h5py does, including [hdf5plugin](https://pypi.org/project/hdf5plugin/) filters:

```{python}
storage = {"chunks": {"time": 1000}, "compression": "lzf", "shuffle": True}
...
[`get_storage`](`dascore.io.get_storage`) returns the storage options class for a format, or None if it has none. Write options the writer does not accept, unknown presets, and chunk dimensions the patch lacks raise a [`ParameterError`](`dascore.exceptions.ParameterError`); an invalid storage field raises pydantic's `ValidationError`, a `ValueError`.
...
[`dc.scan_payloads(...)`](`dascore.scan_payloads`) returns [`PatchMeta`](`dascore.PatchMeta`) objects -- patch metadata without the data -- with each formatter's full `CoordManager`, attrs, and dtype. With `snap=False`, formats that store per-sample coordinates expose their exact values:
