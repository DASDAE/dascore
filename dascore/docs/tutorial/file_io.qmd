...
[`dc.scan_payloads(...)`](`dascore.scan_payloads`) returns data-less patches with each formatter's full `CoordManager`, attrs, and dtype. With `snap=False`, formats that store per-sample coordinates expose their exact values:
