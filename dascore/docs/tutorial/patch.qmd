...
[`Patch.unselect`](`dascore.Patch.unselect`) removes what `select` would keep. Removing an interior range leaves a hole in the coordinate, which `get_discontinuities` reports as a seam.

```{python}
outside = patch.unselect(distance=(50, 200))
assert outside.get_coord("distance").step == patch.get_coord("distance").step
assert outside.get_coord("distance").holes
assert not outside.get_coord("distance").evenly_sampled
```
