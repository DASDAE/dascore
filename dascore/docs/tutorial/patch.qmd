...
An attribute is a single value: a number, a string, a time, a duration, a quantity. Anything holding more than one value is refused, and belongs on the patch as a coordinate instead:

```{python}
import numpy as np

per_channel = np.arange(patch.shape[0], dtype=float)
located = patch.update_coords(gauge_length=("distance", per_channel))
print(located.get_coord("gauge_length").shape)
```

A value riding no dimension is spelled `(None, values)`. See [PatchAttrs](../notes/patch_attrs.qmd) for the rule and what a reader does with a file which stored something else.
