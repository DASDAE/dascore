...
Substitute the selected environment's executable for `python`. When the operation's name is unfamiliar, search the Markdown directory reported by `doc` using your editor or agent's file-search tools. Search API names, keyword metadata, and scientific terms such as `filter`, `bandpass`, `inventory`, or `geometry`, then read the relevant sections. Point the search explicitly at that directory, since the user cache may be outside the project workspace. The [local documentation guide](../tutorial/local_documentation.qmd) includes ripgrep examples.
