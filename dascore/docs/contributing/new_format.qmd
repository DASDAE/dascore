...
Writing is optional: implement `write(self, spool, resource, **kwargs)` when supported. Writer options must be named parameters of `write`; `dc.write` refuses any others. A format offering storage options (compression, chunking) defines a [`BaseStorage`](`dascore.io.BaseStorage`) subclass, sets it as the class's `storage_cls`, takes a `storage` parameter, and coerces it with `storage_cls._coerce(storage)`. Two class attributes describe what a writer can hold: `multi_patch_write` when one file takes several patches, and `segmented_write` when a patch may keep gapped dimensional coordinates; without it, `dc.write` splits or refuses such patches.
