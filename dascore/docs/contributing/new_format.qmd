...
[`get_coord(data=values)`](`dascore.core.coords.get_coord`) reads labels which nearly restate one grid as that grid, which is the snapped path; [`NumericND.from_array`](`dascore.core.coords.NumericND.from_array`) keeps every label it is given and is the exact one. Declare header-defined grids with `get_coord(start=..., step=..., shape=...)`; those axes may treat `snap=False` as a no-op. Snapping changes coordinate labels, not the number of written samples. Both modes must trim unwritten storage rows.
