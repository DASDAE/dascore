...
Preserve per-sample labels with [`get_coord(data=values)`](`dascore.core.coords.get_coord`). Declare header-defined grids with `get_coord(start=..., step=..., shape=...)`; those axes may treat `snap=False` as a no-op. To fit measured labels, use [`snap_stored_coord`](`dascore.io.utils.snap_stored_coord`), which honors the requested dimensions and configured `snap_tolerance`. Unrecognized grids remain stored as arrays, so prefer targeted files over whole-directory exact scans.
