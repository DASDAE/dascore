...
An attr is one value, so a reader states one: [`maybe_get_items`](`dascore.utils.misc.maybe_get_items`) and [`extract_h5_attrs`](`dascore.utils.hdf5.extract_h5_attrs`) collapse the length-1 array HDF5 usually spells a scalar with and read bytes as text. Where a file genuinely names several things, say them as one value — ProdML joins several facility names with commas — or put them on the patch as a coordinate. A value which still holds more than one reaches `PatchAttrs`, which skips it with a warning rather than failing the read; see [PatchAttrs](`dascore/docs/notes/patch_attrs.qmd`).
