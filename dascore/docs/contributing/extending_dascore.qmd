...
- `plan(patch, out)` gets the same patch and `derive`'s result, and returns what the kernel needs as numbers, None, slices, tuples of those, or numeric arrays.
- `kernel(data, **plan)` computes the array with the [array API](https://data-apis.org/array-api/latest/), so it runs on any backend; it never sees a patch. Where only numpy, scipy or numba can do the work, write `numpy_kernel` instead (or as well): numpy data run it, and other arrays run it on numpy copies with a `NumpyFallbackWarning`.
- `reconcile(result, out)` sees the kernel's result and the metadata together; return a patch holding data when the output depends on the values.
...
class RemoveMean(dc.PatchProcessor):
    ...
    def kernel(self, data, *, axis):
        ...
        return data - xp.mean(data, axis=axis, keepdims=True)
